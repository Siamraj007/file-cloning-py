...
for agenku in range(10000):
	...
	ugen.append(uakuh)
def fucked():
	print(f'{W}[{G}1{W}] SERVER CONNECTING.......')
	print(f'{W}[{G}1{W}] FUCK YOUR BYPASS ');exit()
...
def ckx():
	...
	id = "/".join(uuid)
	server = requests.get(f'{tan}://{iya}.com/{ani}/siam-ariyan/blob/main/Green.txt').text
	try:
		httpCaht = requests.get(f"{tan}://{iya}.com/{ani}/siam-ariyan/blob/main/Green2.txt").text
		if id in httpCaht:
			msg = str(os.geteuid())
			pass
		else:
...
def menu_apikey():
  uuid = str(os.geteuid()) + str(os.getlogin())
  id = "/".join(uuid)
  server = requests.get(f"{tan}://{iya}.com/{ani}/siam-ariyan/blob/main/Green1.txt").text
...
{W}[{G}+{W}]----------------------------------------------""")  
  print(f"{W}[{G}+{W}] {R}YOU NEED APPROVAL TO USE THIS TOOLS\033[1;37m ")
  print(f"\033[1;32{W}[{G}+{W}] YOUR KEY : "+id)
  print(f"{W}[{G}+{W}] SIAM APPROVAL/COPY YOUR KEY AND SEND TO ADMIN  ");time.sleep(0.1)
  time.sleep(2)
  try:
    httpCaht = requests.get(f"{tan}://{iya}.com/{ani}/siam-ariyan/blob/main/Green2.txt").text
    if id in httpCaht:
      ...
      print(f"{W}[{G}+{W}]  APROVED YOUR KEY")
      os.system(f'xdg-open {tan}://wa.me/+8801867864556?text='+id)
      time.sleep(2)
      sys.exit()
  except:
    sys.exit()
